...
imports Filtered_Measure Measure_Space_Supplement
begin      
...
text \<open>The natural filtration induced by a stochastic process \<^term>\<open>X\<close> is the filtration generated by all events involving the process up to the time index \<^term>\<open>t\<close>, i.e. \<^term>\<open>\<Sigma> t = \<sigma>({X s | s. s \<le> t})\<close>.\<close>
...
text \<open>In order to show that the natural filtration constitutes a filtered sigma finite measure, we need to provide a countable exhausting set in the preimage of \<^term>\<open>X t\<^sub>0\<close>.\<close>

lemma (in sigma_finite_measure) sigma_finite_filtered_measure_natural_filtration:
  assumes "stochastic_process M t\<^sub>0 X"
      and exhausting_set: "countable A" "(\<Union>A) = space M" "\<And>a. a \<in> A \<Longrightarrow> emeasure M a \<noteq> \<infinity>" "\<And>a. a \<in> A \<Longrightarrow> \<exists>b \<in> borel. a = X t\<^sub>0 -` b \<inter> space M"
...
proof (unfold_locales)
  ...
  show "\<And>i j. \<lbrakk>t\<^sub>0 \<le> i; i \<le> j\<rbrakk> \<Longrightarrow> sets (natural_filtration M t\<^sub>0 X i) \<subseteq> sets (natural_filtration M t\<^sub>0 X j)" using filtered_measure_natural_filtration.subalgebra_F by (simp add: subalgebra_def)
qed (auto intro: stochastic_process.subalgebra_natural_filtration assms(1))
...
proof
  interpret stochastic_process M t\<^sub>0 X by (rule assms)
  show "t\<^sub>0 \<le> i \<Longrightarrow> subalgebra M (natural_filtration M t\<^sub>0 X i)" for i using subalgebra_natural_filtration by blast
  show "\<lbrakk>t\<^sub>0 \<le> i; i \<le> j\<rbrakk> \<Longrightarrow> sets (natural_filtration M t\<^sub>0 X i) \<subseteq> sets (natural_filtration M t\<^sub>0 X j)" for i j using filtered_measure_natural_filtration.subalgebra_F by (simp add: subalgebra_def)
qed
...
sublocale stochastic_process \<subseteq> adapted_natural: adapted_process M "natural_filtration M t\<^sub>0 X" t\<^sub>0 X by (unfold_locales) (auto simp add: natural_filtration_def intro: random_variable measurable_family_vimage_algebra) 
...
lemma scaleR_right_const'_predictable: "predictable_process M F t\<^sub>0 (\<lambda>i \<xi>. c *\<^sub>R (X i \<xi>))" 
  by (fastforce intro: scaleR_right_predictable predictable_process_const_const)
