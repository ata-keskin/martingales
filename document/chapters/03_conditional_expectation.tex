...
The formalization of the averaging theorem was part of our development\footnote{\texttt{Sigma\_Finite\_Measure\-\_Addendum.averaging\_theorem}}. At the beginning of our proof, we assumed $\mu(\Omega) < \infty$ without loss of generality. This is only possible since we assumed the measure space in question to be $\sigma$-finite. To simplify the formalization of proofs employing this argument, we have introduced the following induction scheme
...
When working with an ordering on $E$, we may need to concern ourselves with non-negative simple functions. For this goal, we have another induction scheme\footnote{\texttt{Bochner\_Integration\_Addendum.integrable\_simple\_function\_induct\_nn}}.\label{ind:simple_nn}
...
A major advantage of defining the conditional expectation this way is that it allows us to make statements about its measurability and integrability, without needing to show existence or uniqueness. The following formalized lemmas reflect this.
