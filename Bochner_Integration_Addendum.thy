...
lemma\<^marker>\<open>tag important\<close> simple_integrable_function_induct_nn[consumes 3, case_names cong indicator add, induct set: simple_function]:
...
proof cases
  ...
  obtain s where *: "\<And>i. simple_function M (s i)" 
      ...
      using borel_measurable_simple_function simple integrable by (intro integral_dominated_convergence[OF max(1) _ integrable_norm[OF integrable_scaleR_right], of _ 2 f], blast+)
    {
      ...
      proof (induct rule: simple_integrable_function_induct_nn)
        case (cong f g)                   
