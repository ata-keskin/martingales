...
proof -
  ...
  proof -
    ...
    proof-
      have "LINT x|M. dist (cond_exp M F (s i) x) (cond_exp M F (s j) x) = LINT x|M. norm (cond_exp M F (s i) x - cond_exp M F (s j) x)" using dist_norm by meson
      also have "... = LINT x|M. norm (cond_exp M F (\<lambda>x. s i x - s j x) x)" using cond_exp_diff'[OF has_cond_exp_simple(1,1)[OF assms(2,3)], THEN AE_symmetric, of i j] by (intro integral_cong_AE, auto)
      ...
      also have "... = LINT x|M. norm (s i x - s j x)" using assms(2) fin_sup
        apply (subst set_integral_space[OF integrable_cond_exp, symmetric])
        apply (subst set_integral_space[OF dist_norm_integrable, symmetric])
        apply (intro has_cond_expD(1)[OF has_cond_exp_simple, of _ "space M", symmetric])
        apply (metis simple_function_compose1 simple_function_compose2, simp)
        by (metis sets.top subalg subalgebra_def)
...
lemma cond_exp_integral:
  ...
  using has_cond_expD(1)[OF has_cond_exp_cond_exp, OF assms] by argo
