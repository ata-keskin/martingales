...
proof -
  show "space (natural_filtration M t\<^sub>0 X t) = space M" unfolding natural_filtration_def space_sigma_gen ..
...
lemma (in finite_measure) sigma_finite_filtered_measure_natural_filtration:
  assumes "stochastic_process M t\<^sub>0 X"
  shows "sigma_finite_filtered_measure M (natural_filtration M t\<^sub>0 X) t\<^sub>0"
proof (intro sigma_finite_filtered_measure_natural_filtration[OF assms(1), of "{space M}"])
  have "space M = X t\<^sub>0 -` UNIV \<inter> space M" by blast
  thus "\<And>a. a \<in> {space M} \<Longrightarrow> \<exists>b\<in>sets borel. a = X t\<^sub>0 -` b \<inter> space M" by force
qed (auto)
